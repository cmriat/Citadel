# Citadel Release 用户指南

详细的使用说明和技术文档。

## 目录

1. [快速入门](#快速入门)
2. [环境变量配置](#环境变量配置)
3. [下载CLI详解](#下载cli详解)
4. [转换CLI详解](#转换cli详解)
5. [合并CLI详解](#合并cli详解)
6. [上传CLI详解](#上传cli详解)
7. [数据格式说明](#数据格式说明)
8. [技术原理](#技术原理)
9. [故障排除](#故障排除)

---

## 快速入门

5分钟完成环境配置和首次数据处理。

### 步骤1：安装环境

详细安装步骤请参考 [INSTALL.md](./INSTALL.md)。

```bash
# 确保已安装pixi
curl -fsSL https://pixi.sh/install.sh | bash

# 进入项目目录并安装依赖
cd Citadel
pixi install

# 复制环境变量配置文件
cp .env.example .env
# 根据您的环境修改 .env 文件
```

### 步骤2：配置mc工具

确保mc工具已配置BOS访问：

```bash
# 检查mc是否可用（mc会从PATH或环境变量MC_PATH查找）
mc --version

# 如果未配置，添加BOS别名
mc alias set bos <endpoint> <access-key> <secret-key>

# 验证连接
mc ls bos/
```

### 步骤3：下载数据

```bash
pixi run download \
  --bos-path "srgdata/robot/raw_data/upload_test/online_test_hdf5_v1/fold_laundry/" \
  --local-path "./test_data/download_test/"
```

### 步骤4：转换数据

```bash
pixi run convert \
  --input-dir "./test_data/download_test/" \
  --output-dir "./test_data/convert_test/"
```

### 步骤5：合并数据集（可选）

如果转换后生成了多个独立episode，需要合并为单个数据集：

```bash
pixi run merge \
  --sources ./test_data/convert_test/episode_* \
  --output "./test_data/merged_dataset/" \
  --state-max-dim 14 \
  --action-max-dim 14
```

### 步骤6：上传数据

将合并后的数据集上传到BOS：

```bash
pixi run upload \
  --local-dir "./test_data/merged_dataset/" \
  --bos-path "srgdata/robot/lerobot_data/test/"
```

完成！现在数据已经从BOS下载、转换、合并并上传回BOS的LeRobot目录。

完成！转换后的LeRobot v2.1格式数据已保存在输出目录。

---

## 环境变量配置

项目支持通过环境变量配置所有参数，避免硬编码路径和魔法数字。

### 配置方法

复制 `.env.example` 为 `.env` 并根据需要修改：

```bash
cp .env.example .env
```

### 后端配置项

| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `API_HOST` | `0.0.0.0` | API服务器监听地址 |
| `API_PORT` | `8000` | API服务器端口 |
| `MC_PATH` | 自动检测 | mc可执行文件路径 |
| `BOS_ALIAS` | `bos` | mc配置的BOS别名 |
| `BOS_TEST_PATH` | `srgdata/` | BOS连接测试路径 |
| `DEFAULT_CONCURRENCY` | `10` | 默认并发数 |
| `DEFAULT_FPS` | `25` | 默认视频帧率 |
| `DEFAULT_ROBOT_TYPE` | `airbot_play` | 默认机器人类型 |
| `DEFAULT_TASK_NAME` | `Fold the laundry` | 默认任务描述 |
| `DEFAULT_PARALLEL_JOBS` | `4` | 默认并行转换任务数 |
| `STATE_MAX_DIM` | `14` | 状态向量最大维度 |
| `ACTION_MAX_DIM` | `14` | 动作向量最大维度 |
| `DB_PATH` | `backend/data/tasks.db` | 数据库文件路径 |
| `TIMEOUT_MC_CHECK` | `30` | mc连接检查超时(秒) |
| `TIMEOUT_CONVERT_FILE` | `300` | 单文件转换超时(秒) |
| `CORS_ORIGINS` | `http://localhost:5173` | 允许的CORS来源 |

### 前端配置项

前端配置在 `frontend/.env` 文件中：

| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `VITE_API_PORT` | `8000` | 后端API端口 |
| `VITE_DEV_PORT` | `5173` | 前端开发服务器端口 |
| `VITE_DEFAULT_CONCURRENCY` | `10` | 默认并发数 |
| `VITE_API_TIMEOUT` | `30000` | API请求超时(毫秒) |

### mc路径查找优先级

系统按以下顺序查找mc可执行文件：

1. 环境变量 `MC_PATH` 指定的路径
2. 用户目录 `~/bin/mc`
3. 系统PATH中的 `mc` 命令

### 配置示例

**.env 文件示例：**
```bash
# API服务器配置
API_HOST=0.0.0.0
API_PORT=8000

# mc工具配置（如果mc在非标准位置）
MC_PATH=/usr/local/bin/mc
BOS_ALIAS=bos

# 默认参数
DEFAULT_CONCURRENCY=15
DEFAULT_FPS=30

# 数据维度（根据机器人类型调整）
STATE_MAX_DIM=14
ACTION_MAX_DIM=14
```

---

## 下载CLI详解

### 命令格式

```bash
pixi run download [OPTIONS]
```

### 完整参数列表

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--bos-path` | string | **必填** | BOS远程路径 |
| `--local-path` | string | **必填** | 本地保存目录 |
| `--concurrency` | int | 环境变量 `DEFAULT_CONCURRENCY` 或 `10` | 并发下载数（推荐10-15） |
| `--mc-path` | string | 环境变量 `MC_PATH` 或自动检测 | mc可执行文件路径 |

### 使用示例

**基础用法：**
```bash
pixi run download \
  --bos-path "srgdata/robot/raw_data/your_task/" \
  --local-path "./data/raw/"
```

**自定义路径：**
```bash
pixi run download \
  --bos-path "srgdata/robot/raw_data/other_task/" \
  --local-path "/data/project/raw/"
```

**高并发下载：**
```bash
pixi run download --concurrency 15
```

### 输出说明

下载过程中会显示：
- BOS连接状态
- 文件下载进度（逐个显示）
- 完成统计（文件数、总大小、速度）

```
================================================================================
📥 BOS下载工具 - Citadel Release
================================================================================
BOS路径: bos/srgdata/...
本地路径: /home/user/data/
并发数: 10
================================================================================

✓ mc工具已找到: /home/maozan/mc

🔗 检查BOS连接...
✓ BOS连接成功
✓ 本地目录已创建

🚀 开始下载...

[ 1] ✓ episode_0001.h5
[ 2] ✓ episode_0002.h5
...

================================================================================
📊 下载完成统计
================================================================================
下载文件数: 18
总大小: 246.00 MB
总耗时: 16.0秒
平均速度: 15.22 MB/s
================================================================================
```

---

## 转换CLI详解

### 命令格式

```bash
pixi run convert [OPTIONS]
```

### 完整参数列表

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--input-dir` | string | **必填** | 输入HDF5目录 |
| `--output-dir` | string | **必填** | 输出LeRobot目录 |
| `--robot-type` | string | 环境变量 `DEFAULT_ROBOT_TYPE` 或 `airbot_play` | 机器人类型标识 |
| `--fps` | int | 环境变量 `DEFAULT_FPS` 或 `25` | 输出视频帧率 |
| `--task` | string | 环境变量 `DEFAULT_TASK_NAME` 或 `Fold the laundry` | 任务描述 |
| `--parallel-jobs` | int | 环境变量 `DEFAULT_PARALLEL_JOBS` 或 `4` | 并发转换数 |
| `--file-pattern` | string | `episode_*.h5` | 文件匹配模式 |

### 使用示例

**基础用法：**
```bash
pixi run convert \
  --input-dir "./data/raw/" \
  --output-dir "./data/lerobot/"
```

**指定机器人类型和任务：**
```bash
pixi run convert \
  --robot-type "custom_robot" \
  --task "Pick and place objects"
```

**高并发转换：**
```bash
pixi run convert --parallel-jobs 8
```

**转换特定文件：**
```bash
pixi run convert --file-pattern "episode_000[1-5].h5"
```

### 输出说明

```
================================================================================
🔄 HDF5批量转换工具 - Citadel Release
================================================================================
输入目录: /home/user/data/raw/
输出目录: /home/user/data/lerobot/
机器人类型: airbot_play
FPS: 25
任务: Fold the laundry
并发数: 4
================================================================================

📦 找到 18 个HDF5文件:
  - episode_0001.h5
  - episode_0002.h5
  ...

🚀 开始批量转换（4个并发任务）...

[ 1/18] ✓ episode_0001.h5           (1.2s)
[ 2/18] ✓ episode_0002.h5           (0.9s)
...

================================================================================
📊 转换完成统计
================================================================================
总文件数: 18
✓ 成功: 18
⏱️  总耗时: 17.0秒 (平均 1.0秒/文件)
================================================================================
✅ 所有文件转换成功！
================================================================================
```

---

## 合并CLI详解

### 命令格式

```bash
pixi run merge [OPTIONS]
```

### 完整参数列表

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--sources` | list[str] | **必填** | 源数据集路径列表（支持通配符） |
| `--output` | string | **必填** | 输出合并数据集路径 |
| `--state-max-dim` | int | 环境变量 `STATE_MAX_DIM` 或 `14` | 状态向量最大维度 |
| `--action-max-dim` | int | 环境变量 `ACTION_MAX_DIM` 或 `14` | 动作向量最大维度 |
| `--fps` | int | 环境变量 `DEFAULT_FPS` 或 `25` | 视频帧率 |
| `--copy-images` | bool | `False` | 是否复制图像文件 |

### 使用示例

**基础用法（合并2个数据集）：**
```bash
pixi run merge \
  --sources "./converted/episode_0001/" "./converted/episode_0002/" \
  --output "./merged_dataset/"
```

**使用通配符合并所有episode：**
```bash
pixi run merge \
  --sources ./converted/episode_* \
  --output "./merged_dataset/" \
  --state-max-dim 14 \
  --action-max-dim 14
```

**指定帧率和复制图像：**
```bash
pixi run merge \
  --sources ./data/episode_* \
  --output "./output/" \
  --fps 30 \
  --copy-images
```

### 功能特性

**自动维度对齐**：
- 自动检测所有源数据集的state/action维度
- 使用零填充统一到指定的最大维度
- 确保合并后数据集维度一致

**索引重排**：
- 自动重新编号episode索引（从0开始）
- 自动重新编号frame索引（每个episode从0开始）
- 保持全局索引的连续性

**元数据合并**：
- 合并所有数据集的meta/info.json
- 合并任务列表（去重）
- 计算总帧数、总视频数、总episodes等统计信息

**视频文件处理**：
- 自动复制所有相机视频文件
- 重新命名以匹配新的episode索引
- 保持原始视频质量

### 输出说明

```
================================================================================
🔀 LeRobot数据集合并工具 - Citadel Release
================================================================================
源数据集数量: 232
  1. ./converted/episode_0001/
  2. ./converted/episode_0002/
  ...
输出路径: ./merged_dataset/
状态向量最大维度: 14
动作向量最大维度: 14
视频帧率: 25
复制图像: 否
================================================================================

📁 验证源数据集...
  ✓ ./converted/episode_0001/
  ✓ ./converted/episode_0002/
  ...

🚀 开始合并数据集...

正在合并 episode 1/232...
正在合并 episode 2/232...
...

================================================================================
✅ 数据集合并完成！
================================================================================
输出位置: ./merged_dataset/

📊 合并后统计:
  总 Episodes: 232
  总帧数: 365518
  总任务数: 1
  总视频数: 696
================================================================================
```

### 使用场景

**场景1：批量转换后合并**
```bash
# 步骤1: 批量转换HDF5文件（生成多个独立episode）
pixi run convert --input-dir "./raw_hdf5/" --output-dir "./converted/"

# 步骤2: 合并所有episode为单个数据集
pixi run merge --sources ./converted/episode_* --output "./final_dataset/"
```

**场景2：合并不同批次的数据**
```bash
# 合并来自不同时间或不同任务的数据
pixi run merge \
  --sources "./batch1/episode_*" "./batch2/episode_*" "./batch3/episode_*" \
  --output "./combined_all/"
```

**场景3：处理不同维度的数据**
```bash
# 自动对齐不同维度的state/action向量
pixi run merge \
  --sources "./robot_a/" "./robot_b/" \
  --output "./mixed_robots/" \
  --state-max-dim 20 \
  --action-max-dim 20
```

### 注意事项

1. **源数据集验证**：所有源路径必须是有效的LeRobot v2.1格式数据集
2. **输出路径覆盖**：如果输出路径已存在，会提示是否覆盖
3. **维度设置**：确保max-dim参数大于等于实际数据的最大维度
4. **磁盘空间**：合并操作会复制视频文件，确保有足够磁盘空间
5. **内存使用**：大量数据合并时注意内存占用

---

## 上传CLI详解

### 命令格式

```bash
pixi run upload [OPTIONS]
```

### 完整参数列表

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--local-dir` | string | **必填** | 本地LeRobot数据集目录 |
| `--bos-path` | string | **必填** | BOS目标路径（不含bos/前缀） |
| `--concurrency` | int | 环境变量 `DEFAULT_CONCURRENCY` 或 `10` | 并发上传数（推荐10-15） |
| `--mc-path` | string | 环境变量 `MC_PATH` 或自动检测 | mc可执行文件路径 |

### 使用示例

**基础用法：**
```bash
pixi run upload \
  --local-dir "./data/merged/" \
  --bos-path "srgdata/robot/lerobot_data/my_dataset/"
```

**指定本地目录和BOS路径：**
```bash
pixi run upload \
  --local-dir "./merged_dataset/" \
  --bos-path "srgdata/robot/lerobot_data/fold_laundry/"
```

**高并发上传：**
```bash
pixi run upload \
  --local-dir "./data/" \
  --bos-path "srgdata/robot/lerobot_data/test/" \
  --concurrency 15
```

**自定义mc路径：**
```bash
pixi run upload \
  --local-dir "./output/" \
  --bos-path "srgdata/robot/datasets/" \
  --mc-path "/usr/local/bin/mc"
```

### 输出说明

```
================================================================================
📤 BOS上传工具 - Citadel Release
================================================================================
本地路径: /home/user/data/merged_dataset/
BOS路径: bos/srgdata/robot/lerobot_data/fold_laundry/
并发数: 10
================================================================================

✓ mc工具已找到: /home/maozan/mc

🔗 检查BOS连接...
✓ BOS连接成功

📁 扫描本地目录...
找到以下目录结构:
  - meta/ (2个文件)
  - data/chunk-000/ (232个文件)
  - videos/chunk-000/ (696个视频文件)

🚀 开始上传...

上传进度: 100% (1856/1856 文件)

================================================================================
📊 上传完成统计
================================================================================
上传文件数: 1856
总大小: 640.00 MB
总耗时: 10.2秒
平均速度: 182 files/s (62.75 MB/s)
================================================================================
```

### 目录结构验证

上传前会自动验证本地目录是否为有效的LeRobot v2.1数据集：

**必需的文件结构：**
```
dataset_directory/
├── meta/
│   ├── info.json          # 必需
│   └── tasks.json         # 必需
├── data/
│   └── chunk-000/
│       └── episode_*.parquet
└── videos/
    └── chunk-000/
        └── episode_*/
            └── *.mp4
```

**验证检查项：**
- ✓ `meta/info.json` 文件存在
- ✓ `meta/tasks.json` 文件存在
- ✓ `data/chunk-000/` 目录存在
- ✓ 至少包含一个 `.parquet` 文件

### 注意事项

1. **BOS路径格式**：不要包含 `bos/` 前缀，工具会自动添加
2. **数据完整性**：确保上传前数据集结构完整
3. **网络稳定性**：大文件上传建议在稳定网络环境下进行
4. **并发控制**：根据网络带宽调整并发数，避免超时
5. **覆盖警告**：如果BOS路径已存在文件，mc mirror会自动覆盖

---

## 数据格式说明

### 输入HDF5结构

本工具支持的HDF5文件格式：

```
episode_XXXX.h5
│
├── images/                          # 图像数据
│   ├── cam_env/
│   │   ├── frames_jpeg              # 环境相机帧 (JPEG压缩)
│   │   ├── timestamp_sec            # 秒级时间戳
│   │   └── timestamp_nanosec        # 纳秒级时间戳
│   ├── cam_left/
│   │   ├── frames_jpeg              # 左手相机帧
│   │   ├── timestamp_sec
│   │   └── timestamp_nanosec
│   └── cam_right/
│       ├── frames_jpeg              # 右手相机帧
│       ├── timestamp_sec
│       └── timestamp_nanosec
│
├── joints/                          # 关节数据 (250Hz)
│   ├── left_master/                 # 左臂主控
│   │   ├── joint1_pos ~ joint6_pos  # 6个关节位置
│   │   ├── eef_gripper_joint_pos    # 末端夹爪
│   │   ├── timestamp_sec
│   │   └── timestamp_nanosec
│   ├── right_master/                # 右臂主控
│   │   └── (同上)
│   ├── left_slave/                  # 左臂从控
│   │   └── (同上)
│   └── right_slave/                 # 右臂从控
│       └── (同上)
│
└── metadata                         # 元数据
```

**数据特点：**

| 数据类型 | 采样率 | 说明 |
|----------|--------|------|
| 图像 | ~26 fps | JPEG压缩，三个相机帧数可能不一致 |
| 关节 | 250 Hz | 高频采样，需要下采样对齐到图像帧率 |

### 输出LeRobot v2.1结构

```
episode_XXXX/
│
├── meta/
│   ├── info.json                    # 数据集元信息
│   │   ├── codebase_version: "v2.1"
│   │   ├── robot_type
│   │   ├── fps
│   │   ├── features (state/action维度等)
│   │   └── ...
│   └── tasks.json                   # 任务描述
│
├── data/
│   └── chunk-000/
│       └── episode_000000.parquet   # 状态/动作数据
│           ├── timestamp
│           ├── frame_index
│           ├── task_index
│           ├── state [14维]
│           └── action [14维]
│
└── videos/
    └── chunk-000/
        └── episode_000000/
            ├── cam_env.mp4          # 环境相机视频
            ├── cam_left.mp4         # 左手相机视频
            └── cam_right.mp4        # 右手相机视频
```

**State/Action结构（14维）：**

| 索引 | 内容 | 说明 |
|------|------|------|
| 0-5 | left_joint1_pos ~ left_joint6_pos | 左臂6关节 |
| 6 | left_gripper_pos | 左夹爪 |
| 7-12 | right_joint1_pos ~ right_joint6_pos | 右臂6关节 |
| 13 | right_gripper_pos | 右夹爪 |

- **State**: 从 slave 数据读取（机器人实际状态）
- **Action**: 从 master 数据读取（人类示教动作）

---

## 技术原理

### 多相机时间对齐

三个相机（cam_env, cam_left, cam_right）的帧数可能不一致。转换过程：

1. **确定基准帧数**：取三个相机中最少的帧数
2. **截断多余帧**：其他相机对齐到基准帧数
3. **保持时间同步**：使用各相机自己的时间戳

### 关节数据对齐

关节数据采样率(250Hz)远高于图像帧率(~26fps)。对齐过程：

1. **读取基准时间戳**：使用图像帧的时间戳作为基准
2. **最近邻插值**：对每个图像帧时间戳，找到最近的关节数据
3. **分别处理**：left_slave和right_slave使用各自的时间戳对齐

**对齐精度**：
- 平均对齐误差：1-9ms
- 最大对齐误差：<30ms

### JPEG图像解码

输入HDF5中的图像以JPEG格式压缩存储。转换过程：

```python
# 读取JPEG字节
jpeg_bytes = f["images/cam_env/frames_jpeg"][frame_idx]

# 解码为PIL图像
image = Image.open(io.BytesIO(jpeg_bytes))

# 转换为numpy数组
rgb_array = np.array(image)
```

---

## 故障排除

### 常见错误

#### 1. "mc命令未找到"

```
❌ 错误: mc命令未找到: /home/maozan/mc
```

**解决方案：**
- 确认mc已安装
- 使用 `--mc-path` 指定正确路径
- 或安装mc: `wget https://dl.min.io/client/mc/release/linux-amd64/mc`

#### 2. "BOS连接失败"

```
❌ BOS连接失败: 连接超时
```

**解决方案：**
- 检查网络连接
- 验证mc别名配置: `mc alias list`
- 重新配置: `mc alias set bos <endpoint> <access-key> <secret-key>`

#### 3. "IndexError: index out of bounds"

```
IndexError: index 7492 is out of bounds for axis 0 with size 7492
```

**原因**：左右臂关节数据点数不一致

**解决方案**：已在最新版本修复。确保使用最新的convert.py脚本。

#### 4. "转换超时"

```
转换超时（>5分钟）
```

**解决方案：**
- 检查HDF5文件是否损坏
- 减少并发数: `--parallel-jobs 2`
- 检查系统资源使用

#### 5. "HDF5文件格式不匹配"

```
KeyError: 'images/cam_env/frames_jpeg'
```

**解决方案：**
- 确认HDF5文件结构符合预期格式
- 使用h5dump检查文件结构: `h5dump -H file.h5`

### 调试技巧

#### 查看HDF5文件结构

```bash
# 使用h5dump查看结构
h5dump -H episode_0001.h5

# 使用Python交互式查看
python -c "
import h5py
with h5py.File('episode_0001.h5', 'r') as f:
    def print_structure(name, obj):
        print(name)
    f.visititems(print_structure)
"
```

#### 单独转换单个文件

```bash
python scripts/convert.py \
  --hdf5-path "test_data/download_test/episode_0001.h5" \
  --output-dir "test_data/debug_test/episode_0001/" \
  --robot-type "airbot_play" \
  --fps 25 \
  --task "Debug test"
```

#### 检查转换结果

```bash
# 查看输出目录结构
ls -la test_data/convert_test/episode_0001/

# 检查parquet文件
python -c "
import pandas as pd
df = pd.read_parquet('test_data/convert_test/episode_0001/data/chunk-000/episode_000000.parquet')
print(df.head())
print(f'Columns: {df.columns.tolist()}')
print(f'Shape: {df.shape}')
"

# 检查视频文件
ffprobe test_data/convert_test/episode_0001/videos/chunk-000/episode_000000/cam_env.mp4
```

---

## 性能优化建议

### 下载优化

- **并发数**：网络良好时使用 `--concurrency 15`
- **分批下载**：大量文件时分批处理，避免中断后重新开始

### 转换优化

- **并发数**：根据CPU核心数调整，推荐 `--parallel-jobs $(nproc)`
- **SSD存储**：输入输出目录使用SSD可显著提升速度
- **内存监控**：高并发转换时注意内存使用

### 典型性能

| 环境 | 下载速度 | 转换速度 |
|------|----------|----------|
| 内网+SSD | 15+ MB/s | 1秒/文件 |
| 外网+HDD | 5-10 MB/s | 2-3秒/文件 |

---

**文档版本**: v0.2.2
**最后更新**: 2026-01-09
